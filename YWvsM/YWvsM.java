...
public class YWvsM {
	...
	public static void main(String[] args) {
		...
		System.out.println(y.name + "様、" + w.name + "様と協力し " + m.name + "を倒せばあなた方の勝利です！頑張って下さい・・・");
		System.out.println();
		...
		while (true) {		//無限ループ
			// 勇者の行動
			if (y.hp > 0 && !y_act) {	//&&(かつ) → 両方の条件が満たされた場合にtrue	ここでは、勇者のHPが0以上かつtrue(!y_act→trueになるので)の場合に実行される
				y_act = y.menu(m);	//魔王の m
			} else if (y.hp <= 0) {
				y_act = true;
			}
			// 魔法使いの行動
			if (w.hp > 0 && !w_act) {	//&&(かつ) → 両方の条件が満たされた場合にtrue	ちなみに ||(または) → どちらか片方の条件さえ満たされればtrue
				w_act = w.menu(y, m);		//勇者の y　魔王の m
			} else if (w.hp <= 0) {
				w_act = true;
			}
			all_act = y_act & w_act;	//true & false → false(両方の条件が満たされなければfalseになる)		true || false → true(どちらか片方の条件が満たされればtrueになる)
			// 魔王の行動
		...
		if (y.hp <= 0 && w.hp <= 0) {
			...
			System.out.println(m.name + "を倒しました！ [COMPLETE]");
		}
	}
}
